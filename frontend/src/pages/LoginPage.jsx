    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const data = await res.json();
                setError(data.message || 'Login failed'); // Changed from alert to setError to match existing error handling
            }
        } catch (err) {
            console.error(err);
            setError('Something went wrong. Is backend running?'); // Kept original error message for consistency
        }
    };
    ...
    return (
        <div className="flex justify-center items-center min-h-screen p-8 bg-[#FFF5F5]">
            <div className="bg-white/90 backdrop-blur-xl border border-pink-100 p-10 rounded-3xl w-full max-w-sm shadow-xl relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-pink-100/50 blur-3xl -z-10 rounded-full" />
                ...
                <h2 className="text-3xl font-black text-center mb-2 tracking-tight text-black">Welcome <span className="text-pink-600">Back</span></h2>
                <p className="text-black text-center mb-8 text-sm font-bold italic opacity-80">Elevate your events with EventFlow</p>
                ...
                <p className="mt-8 text-center text-sm text-gray-600 font-bold">
                    Don't have an account? <Link to="/register" className="text-pink-600 font-black hover:underline transition-all hover:text-pink-700">Join Now</Link>
                </p>
            </div>
        </div>
    );
